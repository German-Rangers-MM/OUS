...
private _sequenceFailed = getMissionConfigValue ["outroFailed", "Default"];
private _sequenceContinue = getMissionConfigValue ["outroContinue", "Default"];
...
switch (toLowerANSI _endingType) do {
	...
	case "end2": { 
		...
		_sequencePath = "scripts\core\MCC_sequence_outro_continued" + _sequenceContinue + ".sqf";
	};	
	// Mission Failed
	case "end3": { 
		_guiText =  "Ist die Mission fehlgeschlagen?";
		_sequencePath = "scripts\core\MCC_sequence_outro_failed" + _sequenceFailed + ".sqf";
	};	
};
